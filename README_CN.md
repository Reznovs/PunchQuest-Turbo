...
在新版 Android 手机上有两个严重问题：**卡顿**（不充电时帧率暴跌，充电时恢复正常）和 **32 位兼容性**。本项目通过直接修改 smali 字节码来解决这些问题，并提供两个版本——一个纯优化版，一个包含 IAP 破解版。
...
### 2. 32 位/64 位兼容 — `AndroidManifest.xml`

**问题**：游戏只包含 `armeabi-v7a`（32 位）的 .so 库。`targetSdkVersion` 设太高会导致 64 位手机优先寻找 `arm64-v8a` 目录（不存在）而崩溃。
